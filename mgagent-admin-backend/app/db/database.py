"""
数据库工厂 - Admin Backend (MySQL only)
"""
...
def _migrate_knowledge_base(bind) -> None:
    """补齐旧库 knowledge_bases 表 + documents.knowledge_base_id + documents.chunk_ids 列（幂等）"""
    ...
    try:
        ...
        if insp.has_table("documents"):
            ...
            if "chunk_ids" not in columns:
                ...
                bind.commit()
                print("[migrate] added documents.chunk_ids column")
